...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      const checkUser = await axios.get(`http://localhost:5000/customerinfo/customer-infos?email=${email}`);
      if (checkUser.data.exists) {
        toast.error('User Already Exists!');
      } else {
        const response = await axios.post('http://localhost:5000/customerinfo/customer-info', {
          name,
          email,
          phone,
          password,
